...
function stateDigits(raw){
  return raw.flatMap(value=>String(Math.min(99,Math.round(Math.abs(value)/11633*99))).padStart(2,'0').split('').map(Number));
}
...
export default function LambdaCube(){
  ...
  const target={sinX:0,cosX:1,sinY:0,cosY:1,sinZ:0,cosZ:1};
  ...
  const cubeScene={render({ctx,width,height,dt,quality}){
    ...
    glow()?withGlow(ctx,7,draw,{alpha:.2,quality:quality??1,mode:'fast'}):draw();
    drawVignette(ctx,{width,height,strength:.22});
  }};

  function applyState(data){
    target.sinX=data.sin[0];target.cosX=data.cos[0];target.sinY=data.sin[1];target.cosY=data.cos[1];target.sinZ=data.sin[2];target.cosZ=data.cos[2];
    setMetrics(data);
    const previous=lambdaDigits(),next=stateDigits(data.raw);
    setLambdaTransition(transitionFor(previous,next,++lambdaSeq));setLambdaDigits(next);
  }
