...
import RewriteComputer from './RewriteComputer.jsx';

const TAU=Math.PI*2;
const META={
  rule110:{title:'Rule 110',equation:'next = rule110(left, center, right)',note:'continuous cellular computation / no generation reset'},
  rewrite:{title:'Auth Key Computer',equation:'Shared Visual Model checkpoint + server entropy → HMAC-SHA-256 key',note:'every key is anchored to the seeded 24/7 shared world'},
...
export default function MathDisplay(props){const meta=()=>META[props.kind]||META.rule110;return <main class="math-screen"><header class="math-head"><h1>{meta().title}</h1><p>{meta().equation}</p><small>{meta().note}</small></header><div class="math-stage"><Switch fallback={<Rule110/>}><Match when={props.kind==='rule110'}><Rule110/></Match><Match when={props.kind==='rewrite'}><RewriteComputer/></Match><Match when={props.kind==='fourier'}><FourierMachine/></Match><Match when={props.kind==='complex'}><ComplexPlane/></Match><Match when={props.kind==='lorenz'}><LorenzSystem/></Match><Match when={props.kind==='modular'}><ModularCircle/></Match></Switch></div></main>}
