...
export default function DisplayLab(){
  ...
  const matrixScene={render({ctx,width,height,time,quality}){const t=time*speed(),step=Math.floor(t*7);if(step!==matrixStep){matrixStep=step;matrixRaster=scrollDotRaster(dotMessage,28,step)}const q=quality??1,breath=pulse(t,1.15),values=i=>matrixRaster.values[i]?(.72+.28*pulse(t,1.15,(i%matrixRaster.cols)*.025)):0,draw=()=>drawDotMatrix(ctx,{width,height,cols:matrixRaster.cols,rows:matrixRaster.rows,values,radius:.38,gap:.16,offAlpha:.035,pulse:breath});glow()?withGlow(ctx,7,draw,{alpha:.28,quality:q}):draw();finish(ctx,width,height,t)}};
  const pixelScene={render({ctx,width,height,time,quality}){const t=time*speed(),q=quality??1,cols=q<.68?24:32,rows=q<.68?14:18,values=i=>{const x=(i%cols)/(cols-1),y=Math.floor(i/cols)/(rows-1),dx=x-.5,dy=y-.5,ring=Math.sin(Math.hypot(dx,dy)*34-t*4.1),sweep=Math.sin(x*12+t*2.2)+Math.cos(y*15-t*1.7);return clamp(.5+.28*ring+.13*sweep)},draw=()=>drawPixelGrid(ctx,{width,height,cols,rows,values,gap:1.15,round:1,offAlpha:.02});glow()?withGlow(ctx,5,draw,{alpha:.18,quality:q}):draw();finish(ctx,width,height,t)}};
  const xyScene={render({ctx,width,height,time,quality}){const t=time*speed(),q=quality??1,n=Math.max(280,Math.min(xyX.length,Math.floor(xyX.length*(.5+.5*q))));for(let i=0;i<n;i++){const p=i/(n-1)*TAU*3;xyX[i]=Math.sin(p*3+t*.7);xyY[i]=Math.sin(p*4+t*.91+.45)}drawScopeGraticule(ctx,{width,height,xDiv:8,yDiv:8,alpha:.045,axisAlpha:.13});const draw=()=>drawXYScope(ctx,{width,height,xSamples:xyX,ySamples:xyY,count:n,lineWidth:1.1,alpha:.88,scale:.42});glow()?withGlow(ctx,8,draw,{alpha:.2,quality:q}):draw();finish(ctx,width,height,t)}};
  const highResScene={render({ctx,width,height,time,quality}){const t=time*speed(),q=Math.max(.45,quality??1),cols=Math.round(12+10*q),rows=Math.round(7+6*q),samples=Math.round(480+1320*q);drawVectorField(ctx,(x,y)=>[-y+Math.sin(t*.4)*.25,x+Math.cos(t*.35)*.25],{width,height,xMin:-4,xMax:4,yMin:-2.4,yMax:2.4,cols,rows,alpha:.22,maxLength:.34});const draw=()=>drawFunctionPlot(ctx,x=>Math.sin(x*x*.72-t*.7)*1.15/(1+Math.abs(x)*.08),{width,height,xMin:-4,xMax:4,yMin:-2.4,yMax:2.4,samples,lineWidth:.8,alpha:.95,axes:true});glow()?withGlow(ctx,5,draw,{alpha:.15,quality:q}):draw();finish(ctx,width,height,t)}};
  ...
  return <main class={`display-lab${paused()?' is-paused':''}`}>
    <header class="display-lab-head"><div><small>DISPLAY SYSTEM / PERFORMANCE LAB</small><h1>One renderer. One vsync.</h1><p>Shared scheduling, adaptive resolution, reused signal buffers and high-resolution math displays designed to stay smooth together.</p></div><code>{perf().hz.toFixed(0)} Hz · {perf().active} surfaces · {Math.round(perf().load*100)}% renderer load</code></header>
