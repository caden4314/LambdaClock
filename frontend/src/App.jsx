...
const MENU_ITEMS=[{id:'clock',name:'Clock',note:'Church time'},{id:'display',name:'Display Lab',note:'rendering system'}];
...
function readTime(){
  ...
  const digits=`${two(hour)}${minute}${second}`.split('').map(Number);
...
function routeFromHash(){return globalThis.location?.hash==='#display'?'display':'clock'}

export default function App(){
  const first=readTime();
  const [selected,setSelected]=createSignal(routeFromHash());
  ...
  function selectPage(id){
    ...
    globalThis.history?.pushState?.(null,'',id==='clock'?base:`${base}#${id}`);
  }
  ...
  return <>
    <Show when={selected()==='clock'} fallback={<DisplayLab/>}>
