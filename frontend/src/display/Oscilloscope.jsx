...
export default function Oscilloscope(props){
  ...
  const scene={render({ctx,width,height,time,quality}){
    const sources=props.sources?.length?props.sources:[DEFAULT_SOURCE];
    const sampleCount=Math.max(256,Math.min(1536,Math.trunc(props.sampleCount??Math.max(384,width*1.15))));
    ...
    props.glow===false?draw():withGlow(ctx,8,draw,{alpha:.22,quality});if(props.scanlines)drawScanlines(ctx,{width,height,spacing:4,alpha:.045,offset:time*12});drawVignette(ctx,{width,height,strength:.28});
  }};
  return <DisplayCanvas scene={scene} paused={props.paused} persistence={props.persistence??.86} background={props.background??'#000'} resolutionScale={props.resolutionScale??1} maxDpr={props.maxDpr??2.5} maxPixels={props.maxPixels??1_500_000} adaptiveResolution={props.adaptiveResolution!==false} maxFps={props.maxFps??60} class={props.class} label={props.label??'Oscilloscope display'}/>;
}
