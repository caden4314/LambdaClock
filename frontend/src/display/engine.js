...
export function createDisplayRuntime(canvas,options={}){
  ...
  const state={
    ...
    width:1,height:1,dpr:1,last:now(),elapsed:0,frame:0,fps:0,
    options:{background:'#000',persistence:0,maxDpr:2.5,paused:false,...options}
  };
  ...
  function resize(){
    ...
    const dpr=Math.min(state.options.maxDpr??2.5,Math.max(1,globalThis.devicePixelRatio||1));
    const pixelW=Math.max(1,Math.round(width*dpr)),pixelH=Math.max(1,Math.round(height*dpr));
    if(canvas.width!==pixelW||canvas.height!==pixelH){canvas.width=pixelW;canvas.height=pixelH;state.width=width;state.height=height;state.dpr=dpr;ctx.setTransform(dpr,0,0,dpr,0,0);ctx.clearRect(0,0,width,height);state.scene?.resize?.({width,height,dpr,ctx});}
