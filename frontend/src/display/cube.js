...
export function drawWireCube(ctx,{width,height,rotation,distance=4.4,scale=.31,alpha=.94,lineWidth=1.35,widthScale=1,alphaScale=1,vertices=true}={}){
  ...
  if(vertices){
    for(const point of points){const depth=clamp(point[2]*.25+.5,0,1),r=(1.25+depth*.9)*Math.sqrt(ws);ctx.globalAlpha=alpha*as*(.5+.5*depth);ctx.beginPath();ctx.arc(point[0],point[1],r,0,Math.PI*2);ctx.fill()}
  }
