...
function Counter(){
  ...
  return <Shell title="Counter" subtitle="A number as repeated function application." lambda="N ≡ λf.λx.fⁿx">
    <div class="hero-number">{n()}</div><ProjectLambdaPanel kind="church" value={n()} expression={churchText(n())}/>
...
function Arithmetic(){
  ...
  return <Shell title="Arithmetic" subtitle="Compose Church numerals to add or multiply." lambda={op()==='ADD'?'λm.λn.λf.λx.m f (n f x)':'λm.λn.λf.m (n f)'}>
    ...
    <div class="equation"><span>{a()}</span><b>{op()==='ADD'?'+':'×'}</b><span>{b()}</span><i>=</i><strong>{result()}</strong></div><ProjectLambdaPanel kind="arithmetic" op={op()} a={a()} b={b()} expression={`${op()} · Church(${a()}) · Church(${b()}) → ${result()}`}/>
...
function Fibonacci(){
  ...
  return <Shell title="Fibonacci" subtitle="Transform the pair (a,b) → (b,a+b) repeatedly." lambda="FIB ≡ ITER step ⟨0,1⟩">
    <div class="hero-number">{fib(n())}</div><div class="sequence-strip"><For each={seq()}>{(v,i)=><span class={i()===n()?'active':''}>{v}</span>}</For></div><ProjectLambdaPanel kind="fibonacci" n={n()} expression={`Church(${n()}) · STEP · PAIR(0,1) → ${fib(n())}`}/>
...
function Wave(){
  ...
  return <Shell title="Wave" subtitle="Lambda supplies a composable function; the browser interpolates it smoothly." lambda="W ≡ λphase.λx.sin(phase + kx)">
    <canvas class="wave-canvas" ref={canvas}/><ProjectLambdaPanel kind="wave" expression="λphase.λx.SIN (ADD phase (MUL k x))"/><div class="split-controls three"><Range label="amplitude" min={12} max={95} value={amp()} onInput={setAmp}/><Range label="frequency" min={1} max={6} value={freq()} onInput={setFreq}/><Range label="speed" min={0} max={3} step={.25} value={speed()} onInput={setSpeed}/></div>
  </Shell>;
}
...
function Binary(){
  ...
  return <Shell title="Binary" subtitle="Each bit is a Church boolean in a six-bit list." lambda="BITS ≡ CONS TRUE / FALSE">
    ...
    <div class="equation compact"><strong>{value()}</strong><i>↔</i><span>{bits().join('')}</span></div><ProjectLambdaPanel kind="binary" bits={bits()} expression={`CONS ${bits().map(bit=>bit?'TRUE':'FALSE').join(' · ')} · NIL`}/><Range label="value" min={0} max={63} value={value()} onInput={setValue}/>
  </Shell>;
}
...
function RecursionTree(){
  ...
  return <Shell title="Recursion Tree" subtitle="Visualize recursive unfolding like a fixed-point combinator." lambda="Y ≡ λf.(λx.f(xx))(λx.f(xx))">
    ...
    <ProjectLambdaPanel kind="recursion" mode={mode()} n={n()} expression={`Y · ${mode()==='fib'?'FIB':'FACT'} · Church(${n()})`}/>
...
function Automata(){
  const size=41,centered=()=>Array.from({length:size},(_,i)=>i===Math.floor(size/2)?1:0);const [rows,setRows]=createSignal([centered()]),[running,setRunning]=createSignal(true);let timer;const sample=createMemo(()=>{const row=rows()[rows().length-1],c=Math.floor(size/2);return {left:row[c-1]||0,right:row[c+1]||0,out:(row[c-1]||0)!==(row[c+1]||0)}});
